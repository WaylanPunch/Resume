package com.way.resume.controller;
...
import com.way.resume.service.NewresumeService;
import com.way.resume.tools.WordGenerator;

import sun.security.acl.WorldGroupImpl;

/**
 * NewresumeController类 用于控制所有的操作简历的操作
 */
...
public class NewresumeController {
	...
	/*
	 * basicinfo:用于接收用户输入表单的数据 request:用于接收隐藏表单数据 session:传值
	 */
	public String insertbasic(Basic_info basicinfo, HttpServletRequest request, HttpSession session) {
		String ret = null;
		// 接收隐藏表单hidden的值
	 ...
	 * 三个参数的作用同上个方法
	 */
	public String addEducational(Educational_info edu, HttpServletRequest request, HttpSession session) {
	 ...
	 * 三个参数的作用同上
	 */
	public String addWork(Work_info work, HttpServletRequest request, HttpSession session) {
		String ret = null;
		int userid = Integer.parseInt(request.getParameter("Userid"));
	 ...
	 */
	public String addIntroduce(Basic_info intro, HttpServletRequest request, HttpSession session) {
